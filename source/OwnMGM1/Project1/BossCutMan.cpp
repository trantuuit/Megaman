...
BossCutMan* BossCutMan::getInstance()
{
	if (cutman == 0)
	{
		cutman = new BossCutMan();
	}
	return cutman;
}
BossCutMan::BossCutMan()
{
	...
	fileLogCutman.open("abcFileLox.txt", ios::out | ios::app);
}

void BossCutMan::update()
{
	if (!pauseAnimation)
	...
	if (clapActionCount < 200)
	{
		...
		if (damagedDelayTime.isFinish()) // Nếu hết thời gian delay do bị trúng đạn:
		{
			...
			case CM_CLAP:
				...
				break;
			case CM_ATTACK:
				if (timePerAction.isFinish() && curFrame == 1) // Hết thời gian delay
				{
			...
			if (action == CM_ATTACK && curFrame == 1 && !isThrow)
			{
				CutmanBullet *_cutmanBullet = new CutmanBullet();
				//fileLogCutman << "----Cutman-----:" << action << "\t" << curFrame << "\t" << isThrow << endl;
				isThrow = true;
			}
			// Bị trúng đạn
			if (isDamaged)
...
void BossCutMan::onLastFrameAnimation(int _action)
{
	switch (_action)
	{
...
CUTMAN_ACTION BossCutMan::randomAllAction()
{
	srand(time(NULL));
	int result = rand() % 100;
...
CUTMAN_ACTION BossCutMan::randomActionWithoutRUN()
{
	srand(time(NULL));
	int result = rand() % 100;
...
void BossCutMan::onIntersectRect(MGMBox * otherObject)
{
...
void BossCutMan::onCollision(MGMBox* otherObject, int nx, int ny)
{
...
void BossCutMan::resetHeightWhenChangeAnimation()
{
...
void BossCutMan::setCutmanAction(CUTMAN_ACTION _curAction)
{
	switch (_curAction)
	{
	case CM_RUN:
